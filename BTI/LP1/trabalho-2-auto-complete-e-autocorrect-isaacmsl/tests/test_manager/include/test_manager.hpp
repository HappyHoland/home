...
class TestManager {
    private:
        ...
        struct Entry {
            ...
            string m_fail_msg;        //!< A message to be presented if the test fails (for the old version it could be the line).
            bool m_enabled;    //!< Indicates wheter the test is enabled (default) or not.
            /// Default Ctro
            Entry( string d="no_name", size_t s = 0, result_t r=result_t::UNDEFINED, string fail_msg="", bool e=true )
                : m_desc{ d }, m_seq{ s }, m_result{ r }, m_fail_msg{ fail_msg }, m_enabled{ e }
            { /* empty */ }
        };
    ...
    private:
        ...
        void print_test_result( const std::string &test_name, const Entry &entry ) const
        {
            ...
            if ( entry.m_result == Entry::result_t::SUCCESS )
                ...
                std::cout << "[      "  << "\e[1;31mFAIL\e[0m" << " ] " << entry.m_fail_msg << ".\n";
            else if ( entry.m_result == Entry::result_t::UNDEFINED )
                std::cout << "[ "  << "\e[1;35mUNDEFINED\e[0m" << " ] " << entry.m_fail_msg << ".\n";
        }
    ...
    public:
        ...
        void result( const std::string &key, bool value, const std::string msg );
...
#define EXPECT_TRUE( value, msg ) _tm.result( _test_id, value==true, msg )
#define EXPECT_FALSE( value, msg ) _tm.result( _test_id, value==false, msg )
#define EXPECT_EQ( value1, value2, msg ) _tm.result( _test_id, value1==value2, msg )
#define EXPECT_NE( value1, value2, msg ) _tm.result( _test_id, value1!=value2, msg )
#define EXPECT_GT( value1, value2, msg ) _tm.result( _test_id, value1>value2, msg )
#define EXPECT_GE( value1, value2, msg ) _tm.result( _test_id, value1>=value2, msg )
#define EXPECT_LT( value1, value2, msg ) _tm.result( _test_id, value1<value2, msg )
#define EXPECT_LE( value1, value2, msg ) _tm.result( _test_id, value1<=value2, msg )
